...
# created: Vince Darcangelo 11/05/21
# most recent update: Vince Darcangelo 1/24/25
# \OneDrive - UCB-O365\Documents\oit-fcq\code\R\course-audit\FCQ_Audit01.R
#########################################################################
#'*Load ciw credentials*
#########################################################################

# run ciwpass.R from local, secure filepath

#########################################################################
#'*Update semester variables and load libraries*
#########################################################################
...
#########################################################################
#'*Connect and pull data from ciw*
#########################################################################
...
# limit to 4-digit catalog numbers: 
  ...
  filter(!(ACAD_GRP_CD %in% c('ZOTH', 'CONC', 'DENT')) &
    ...
    !(SBJCT_CD %in% c('CHPM', 'PHSL', 'MEDS', 'PRMD', 'PHMD', 'PALC', 'PHSC', 'TXCL')) &
...
# find/isolate single values (crse/inst pairings with 1 mtg or exact dups)
cmtg2a <- cmtg %>%
  group_by(icpair) %>%
  distinct() %>%
  filter(n() == 1)

# find/isolate duplicate values (distinct dups)
cmtg2b <- cmtg %>%
  group_by(icpair) %>%
  distinct() %>%
  filter(n() >= 2)
...
# find/isolate dups with single Y/N value
cmtg_assist <- cmtg2b1 %>%
...
# find/isolate dups with multiple Y/N value, reduce to Y value
...
# SEQUENCE: ACCOUNT FOR INSTRS NOT INCLUDED IN EVERY MTG PATTERN (e.g., class has multiple mtg patterns, but instr isn't listed in all)
...
while (TRUE) {
  if (nrow(cmtg4x) == 0) {
    ...
    View(cmtg4x)
    stop('Review and fix dups (cmtg4x) before continuing')
  }
}
...
# pull from PS_D_INSTRCTR_ROLE as irole
...
# pull from PS_CU_D_NAMES_II as perstbl
perstbl <- dbGetQuery(con,
  'SELECT PERSON_SID, EMPLID, EFFDT, EFF_STATUS, NAME, LAST_NAME, FIRST_NAME, MIDDLE_NAME, NAME_TYPE, CURRENT_IND, NAME_DISPLAY
...
pers <- perstbl %>%
  ...
  group_by(EMPLID) %>%
  filter(name_pref == max(name_pref)) %>%
  filter(EFFDT == max(EFFDT))

colnames(pers) <- c('PERSON_SID', 'instrPersonID', 'EFFDT', 'EFF_STATUS', 'instrNm_src', 'instrLastNm', 'instrFirstNm', 'instrMiddleNm', 'NAME_TYPE', 'CURRENT_IND', 'DisplayNm', 'name_pref')

j3 <- left_join(j2, pers, by = 'PERSON_SID')

# format names to remove symbols
j3 <- j3 %>%
  mutate(across(c(instrNm_src, instrLastNm, instrFirstNm, instrMiddleNm), gsub, pattern = '<e9>', replacement = 'e')) %>%
  mutate(across(c(instrNm_src, instrLastNm, instrFirstNm, instrMiddleNm), gsub, pattern = '<ed>', replacement = 'i'))
...
# pull from PS_CU_D_EXT_SYSTEM as hr
...
# pull from PS_CU_D_CLASS_ATTR as cattr
...
# pull from PS_CU_D_SCTN_CMBND as cmbsec
...
# pull from PS_F_CLASS_ENRLMT as enr
...
#########################################################################
#'*Format data and prep for course audits'*
#########################################################################
...
dept_key <- read.csv(paste0(folder, 'R_Code\\FCQDept_keys.csv'))
...
###!!!QUALITY CHECK: FIND NEW SUBJECTS!!!################################
# QC: IDENTIFY NEW SUBJECTS NOT YET ASSIGNED AN FCQ DEPT
# quality check to remove dups, should == 0
nna <- clmerge %>%
...
while (TRUE) {
  if (nrow(nna) == 0) {
    ...
    View(nna)
    stop('Review the subjects that have not yet been assigned to an FCQ department, add/fix in FCQDept_keys.csv and rerun code from # load FCQ department keys!')
  }
}
...
#############################################################################
# qc check for missing combstat/sctn_cmbnd_cd
cl4x <- cl4 %>%
  filter(SCTN_CMBND_CD != '' & combStat == '')

cl4xx <- cl4 %>%
  filter(combStat != '' & SCTN_CMBND_CD == '')

cl4xxx <- rbind(cl4x, cl4xx)
...
while (TRUE) {
  if (nrow(cl4xxx) == 0) {
    ...
    View(cl4xxx)
# generate doc for registrar
    appenddt <- Sys.Date()
    sheetdate <- format(appenddt, format = '%m%d%y')
    write.csv(cl4xxx, paste0(folder, 'Data_Requests\\Registrar\\ProbCombSect_', sheetdate, '.csv'), row.names = FALSE)
    print('Review the classes that have not yet been properly combined in FCQ - AIM_ Measurement/Data_Requests/Registrar/ProbCombSect_(sheetdate), and send file to appropriate Registrar office!')
    break
  }
}
...
# create spons_id, _comp_cd, _deptOrgID, _AcadGrp, _AcadOrg, _fcqdept cols
spons1 <- spons %>%
...
nspons3 <- left_join(nspons2, spons8, by = c('INSTITUTION_CD', 'SCTN_CMBND_CD', 'SCTN_CMBND_LD'))
...
nspons5 <- nspons4 %>%
  ...
  mutate(instrNm = instrNm.x) %>%
  select(-c(instrNm.x, instrNm.y))

# recombine spons and nspons data
clist2 <- rbind(spons6, nspons5)

#########################
# qc for missing spons_id
clist2x <- clist2 %>%
...
clist5$instrNum <- sequence(rle(clist5$CLASS_NUM)$lengths)
...
# export c20 file
write.csv(clslistCU_3, paste0(folder, 'CourseAudit_bak\\', term_cd, '\\c20.csv'), row.names = FALSE)
##########################################################
#dbDisconnect(con)
