...
n03 <- n02 %>%
  mutate(indDEPT_RQST = case_when(
    ...
    # BD-GEEN (2018/10/30 Ellen Parrish) FCQs for LEC ONLY, not LAB -- UPDATED Spring 2025 to exclude ENED sections (2025/01/22 Vanessa Dunn)
    crseSec_comp_cd != 'LEC' & deptOrgID == 'CUBLD:BLDR:GEEN' & SBJCT_CD != 'ENED' ~ 1,
    ...
    crseSec_comp_cd != 'LEC' & deptOrgID == 'CUBLD:BLDR:MCEN' ~ 1,
...
n13 <- n12 %>%
  mutate(adminInd = case_when(
    ...
    fcqdept == 'GEEN' & SBJCT_CD != 'ENED' & crseSec_comp_cd != 'LEC' ~ 0,
    campus == 'CE' & SBJCT_CD == 'NCIE' ~ 0,
    campus == 'CE' & SESSION_CD == 'BM9' ~ 0,
  # MC-ANAT do not run (2025/07/16 Jennifer Thurston)
    campus == 'MC' & SBJCT_CD == 'ANAT' ~ 0,
  # MC-IDPT only run FCQs for 7806 and 7810 (2020/07/08 Shawna Cox)
...
#################### exceptions
  ...
  # BD-GEEN courses get FCQs for LEC only per Ellen Parrish 10/30/18 -- UPDATED Spring 2025 to exclude ENED sections (2025/01/22 Vanessa Dunn)
  mutate(n11 = case_when(
    deptOrgID == 'CUBLD:BLDR:GEEN' & SBJCT_CD != 'ENED' & crseSec_comp_cd != 'LEC' ~ 
  ...
  mutate(n13 = case_when(
    deptOrgID == 'CUBLD:BLDR:MCEN' & crseSec_comp_cd != 'LEC' ~ 'No FCQs for non LEC sections')) %>%
...
while (TRUE) {
  if (nrow(n14x) == 0) {
    ...
    View(n14x)
    stop('Review and reconcile YES with FCQNOtE before continuing')
  }
}
