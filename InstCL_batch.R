##########################################################################
# Maintain running log of all evaluated instructors since fall 2017
# use for reference and name/constituent ID changes
# created: Vince Darcangelo, 8/4/22
# most recent update: Vince Darcangelo 7/18/25
# \AIM Measurement - FCQ\R_Code\campus_labs\InstCL_batch.R
##########################################################################
...
file_names <- dir(paste0('C:\\Users\\', userid, '\\UCB-O365\\AIM Measurement - FCQ\\CampusLabs\\Imports\\', term_cd, '\\Instructor\\'))
setwd(file_names)
...
# import existing log
instLog <- read.csv(paste0('C:\\Users\\', userid, '\\UCB-O365\\AIM Measurement - FCQ\\CampusLabs\\Data_Files\\instLog.csv'))

# add new semester (datasplit) data to existing log
updatedLog <- rbind(instLog, datasplit)

# export updated log
write.csv(updatedLog, paste0('C:\\Users\\', userid, '\\UCB-O365\\AIM Measurement - FCQ\\CampusLabs\\Data_Files\\instLog.csv'), row.names = FALSE)

#########################################################################
# PAST SEMESTERS
file_names <- dir(paste0('C:\\Users\\', userid, '\\UCB-O365\\AIM Measurement - FCQ\\CampusLabs\\Imports\\2231\\Instructor\\'))
